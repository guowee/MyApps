...
public class TopicRecAdapter extends BaseAdapter{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.icon_dismiss.setTag(topic);
		holder.icon_dismiss.setOnClickListener(bt_click_listener);
	...
	public OnClickListener bt_click_listener = new OnClickListener() {
		
			@Override
			public void onClick(View v) {
				Topic topic_delete=(Topic) v.getTag();
				boolean is_allow_notice = share_prefenrence_notice.getBoolean("is_allow_notice", false);
				if(is_allow_notice){//不在提醒
					notShow(topic_delete);
				}else {//提醒
					showDialog(topic_delete);
				}
			}

			private void showDialog(final Topic topic_delete) {
			...
			private void  notShow(final Topic topic_delete){
					...
					public void onSuccess(String arg0) {
						...
						try {
							...
							if("1".equals(json.getString("flag"))){
								list_topic.remove(topic_delete);
								notifyDataSetChanged();
							}
