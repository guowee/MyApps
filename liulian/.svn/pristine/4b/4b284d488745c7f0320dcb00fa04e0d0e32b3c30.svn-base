...
public class HistoryActivity extends BaseActivity{
	...
	private void init_topic() {
		...
		topic_adapter = new ListViewAdapter(context, "history");
		...
		mPullRefreshListView.setOnRefreshListener(new OnRefreshListener<ListView>() {
			...
			public void onRefresh(PullToRefreshBase<ListView> refreshView) {
				...
				if (NetworkUtil.dataConnected(context)) {
					...
					MyToast.makeText(HistoryActivity.this, context.getResources().getString(R.string.no_network), Toast.LENGTH_SHORT).show();
				}
			}
		});
		...
		mPullRefreshListView.setOnLastItemVisibleListener(new OnLastItemVisibleListener() {
			...
			public void onLastItemVisible() {
				if (NetworkUtil.dataConnected(context)) {
					if (!have_next_new) {
						MyToast.makeText(HistoryActivity.this, context.getResources().getString(R.string.is_the_last_page), 1).show();
					} else {
						if (loading_task != null) {
							loading_task.cancel(true);
						}
		...
		actualListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int item_position, long id) {
				...
				Topic topic = list_topic_all.get(item_position - 1);
	...
	class LoadingTask extends AsyncTask<String, Integer, List<Topic>> {
		...
		protected List<Topic> doInBackground(String... url) {
			...
			try {
				String urlPath = PathConst.URL_LIST_HISTORY 
						+ "&uid=" + LiuLianApplication.current_user.getUid() 
						+ "&location_x=" + LiuLianApplication.latitude 
						+ "location_y=" + LiuLianApplication.longtitude 
						+ "&last_id=" + last_id;
				...
				if (dir_offline != null) {
					file_local = new File(dir_offline + PathConst.OFFLINE_TOPIC_HISTORY);// 保存最新话题的本地json的文件
				}
		...
		protected void onPostExecute(List<Topic> topic_list) {
			...
			if(list_topic_all.size()==0){
				...
				tip1.setText("你还没有浏览过话题");
				tip2.setText("到发现页收藏你喜欢的话题吧");
			}else{
