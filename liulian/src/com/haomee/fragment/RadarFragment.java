...
import com.haomee.adapter.FlowTypesAdapter;
...
public class RadarFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		if (view == null) {
			...
			view = inflater.inflate(R.layout.fragment_radar, null);
			...
			//bt_login_recent = (TextView) view.findViewById(R.id.bt_login_recent);
			...
			if(topic_type != 0){
				...
				new Handler().postDelayed(new Runnable(){
					...
					public void run() {
						...
						flow_types.setSelection(topicTypes.size()*10);
	...
	private void initTop(){
		...
		view.findViewById(R.id.layout_title).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
		...
		for(int i=0;i<=13;i++){
			...
			type.setIcon_res(ImageUtil.getResIdByName(context, "radar_label_"+i));
			
			topicTypes.add(type);
		}			
		
		flow_types.setAdapter(new FlowTypesAdapter(context, topicTypes));
		flow_types.setSelection(topicTypes.size()*10);
		flow_types.setOnItemSelectedListener(new OnItemSelectedListener() {
			@Override
			public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
				int index = position%CommonConst.topic_types.length;
				text_title.setText(CommonConst.topic_types[index]);
				icon_title.setImageResource(topicTypes.get(index).getIcon_res());
			}
			...
			public boolean onTouch(View v, MotionEvent event) {
				if(event.getAction()==MotionEvent.ACTION_UP){
					...
					new Handler().postDelayed(new Runnable() {
						...
						@Override
						public void run() {
							int index = flow_types.getSelectedItemPosition()%CommonConst.topic_types.length;
	...
	private OnClickListener userListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (users_selected.get(index).getUid().equals(LiuLianApplication.current_user.getUid())) {
				...
				Toast.makeText(RadarFragment.this.getActivity(), "这是你自己哦", 1).show();
	...
	class LoadingTask extends AsyncTask<String, Integer, Boolean> {
		...
		protected Boolean doInBackground(String... url) {
			...
			try {
				String urlPath = PathConst.URL_RADAR_NEARBY 
						+ "&uid=" + LiuLianApplication.current_user.getUid() 
						+ "&location_x=" + LiuLianApplication.latitude + "&location_y=" + LiuLianApplication.longtitude
						+ "&category="+topic_type;
